...
for(i in 1:length(dataModified)){
  ...
  cat("\n", cn[i], "\n")
  summary <- group_by(dataModified, dataModified[grID]) %>%
    summarise(
      count = n(),
      mean = mean(wiek)
    )
...
for(i in 1:length(dataModified)){
  ...
  summary <- group_by(dataModified, dataModified[, grID], dataModified[, genderID], .add = TRUE) %>%
    summarise(
      count = n(),
      mean = format(mean(dataModified[, i]), nsmall = 2),
      max = format(max(dataModified[, i]), nsmall = 2),
      min = format(min(dataModified[, i]), nsmall = 2),
      sd = format(sd(dataModified[, i]), nsmall = 2),
      median = format(median(dataModified[, i]), nsmall = 2),
      iqr = format(IQR(dataModified[, i]), nsmall = 2),
    )
  print(summary)
  cat("\n")
}
